    ...
    if (bookBtn != null) {
        ...
        loginBtn.onclick = function () {
            if (loginBtn.innerHTML == "Login") {
                ...
                localStorage.removeItem("choosenPlace");
    ...
    /**************************/
    var nextToSec = document.getElementById("nextToSec");
    var filmName = document.getElementById("filmName");
    var filmTime = document.getElementById("filmTime");
    var filmDate = document.getElementById("datepicker");
    if (nextToSec != null) {
        nextToSec.onclick = function () {
            if (filmName.innerHTML != "" && filmTime.innerHTML != "") {
                var chosenFilm = {
                    filmName: filmName.innerHTML,
                    filmTime: filmTime.innerHTML,
                    filmDate: filmDate.value
                }
                console.log(chosenFilm)
                localStorage.setItem("chosenFilm", JSON.stringify(chosenFilm));
                window.location.href = "book2.html"
            } else {
                alert("Choose your film and it's time")
            }
        }
    }
    /**************************************/
    var nextToThird = document.getElementById("nextToThird");
    var price = document.getElementById("price");
    var want = ``;

    if (nextToThird != null) {
        nextToThird.onclick = function () {
            var numOfTickets = 0;
            var wantedArr = [];
            var choosenPlace = Array.from(document.getElementsByClassName("checked-place"));
            var arr = Array.from(choosenPlace[1].children)

            for (let i = 0; i < arr.length - 1; i++) {
                wantedArr.push(arr[i]);
            }

            for (let i = 0; i < wantedArr.length; i++) {

                if (i < wantedArr.length - 1) {
                    want += `${wantedArr[i].innerHTML} ,`
                    numOfTickets++;
                } else {
                    want += `${wantedArr[i].innerHTML}`;
                    numOfTickets++;
    ...
    /******************************/
    var ticketsNum = document.getElementById("numOfTickets");
    var totalPrice = document.getElementById("totalPrice");
    if (ticketsNum != null) {
        ticketsNum.innerHTML = localStorage.getItem("numOfTickets");
        totalPrice.innerHTML = localStorage.getItem("price");
        var tootal = totalPrice.innerHTML;
        localStorage.setItem("tootal", tootal);
    }
    /******************************/
    var fee = document.getElementById("fee");
    var numOfTickets2 = document.getElementById("numOfTickets2");
    var ticketsPrice2 = document.getElementById("ticketsPrice2")
    var total = document.getElementById("total");

    if (fee != null) {
        fee.innerHTML = localStorage.getItem("numOfTickets")+"$";
        numOfTickets2.innerHTML = localStorage.getItem("numOfTickets");
        ticketsPrice2.innerHTML = localStorage.getItem("price");
        var x = localStorage.getItem("price");
        var y = x.slice(1);
        console.log(y)
        total.innerHTML = Number(localStorage.getItem("numOfTickets")) + Number(y) + "$";
        var tootal = total.innerHTML;
        localStorage.setItem("tootal", tootal);
    }
    ...
    if (filmName != null) {
        var chosenFilm = JSON.parse(localStorage.getItem("chosenFilm"));
        date4.innerHTML = chosenFilm.filmDate;
        time4.innerHTML = chosenFilm.filmTime;
        filmName.innerHTML = chosenFilm.filmName;
        price4.innerHTML = localStorage.getItem("tootal");
        sits.innerHTML = localStorage.getItem("choosenPlace");
    }
    /**************************/
    var moviesH = document.getElementById("movies-h");
    var postersArr = document.getElementsByClassName("poster");
    var movie__title = document.getElementsByClassName("movie__title");
    var postars = [];
    var moviesName = [];
    if (moviesH != null) {
        postars = Array.from(postersArr)
        localStorage.setItem("Posters", JSON.stringify(postars));
        for (var i = 0; i < 10; i++) {
            moviesName.push(Array.from(movie__title)[i].outerText)
        }
        localStorage.setItem("moviesName", JSON.stringify(moviesName))
    }

    function movieDetails(index) {
        localStorage.setItem("index", index);
        window.location.href = "movie-details.html"
    }
    /****************************/
    var jjjjj = document.getElementById("jjjjj");
    ...
    if (jjjjj != null) {
        ...
        var nnn = JSON.parse(localStorage.getItem("moviesName"));
        jjjjj.innerHTML = nnn[index - 1];
        moviePoster.setAttribute("src", `images/Movies Poster/${index}.jpg`)
        var storyLine = ["Rewind to the 1980s as Wonder Woman's next big screen adventure finds her facing two all-new foes: Max Lord and The Cheetah.", "One Night in Miami is a fictional account of one incredible night where icons Muhammad Ali, Malcolm X, Sam Cooke, and Jim Brown gathered discussing their roles in the civil rights movement and cultural upheaval of the 60s.", "A young woman, traumatized by a tragic event in her past, seeks out vengeance against those who cross her path.", "A Civil War veteran agrees to deliver a girl, taken by the Kiowa people years ago, to her aunt and uncle, against her will. They travel hundreds of miles and face grave dangers as they search for a place that either can call home", "Old woodcarver Geppetto's puppet creation, Pinocchio, magically comes to life with dreams of becoming a real boy. Easily led astray, Pinocchio tumbles from one misadventure to another as he is tricked, kidnapped and chased by bandits.", " After receiving life-altering news, a couple finds unexpected support from their best friend, who puts his own life on hold and moves into their family home, bringing an impact much greater and more profound than anyone could have imagined", "This is the story of young mother Sandra who escapes her abusive husband and fights back against a broken housing system. She sets out to build her own home and, in the process, rebuilds her life and re-discovers herself.", "Behind the gates of a palm tree-lined fantasyland, four residents of America's largest retirement community, The Villages, FL, strive to find solace and meaning.", `A modern Western inspired by the real-life "no man's land" areas along the Texas-Mexico border. While out on a father-son border patrol, Jackson accidentally kills a Mexican immigrant boy. When Jackson's father tries taking the blame, Jackson flees south on horseback, becoming a gringo "illegal alien" in Mexico. Chased by Texas Rangers and Mexican federales, Jackson journeys across Mexico to seek forgiveness from the dead boy's father only to fall in love with the land he was taught to hate.`, "A rancher on the Arizona border becomes the unlikely defender of a young Mexican boy desperately fleeing the cartel assassins who have pursued him into the U.S."]
        var movie__describe = document.getElementById("movie__describe");
        movie__describe.innerHTML=storyLine[index-1];
    ...
    function init_Contact() {
        ...
        var mapOptions = {
            scaleControl: true,
            center: new google.maps.LatLng(29.99346181949527, 31.31132980418111),
            zoom: 15,
